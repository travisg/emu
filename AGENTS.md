...
- The 703 mounts disc images the same way: `ray703-disc0.img`..`ray703-disc3.img` from the current directory onto the 74601 disc controller's four units (`src/dev/disc74601.rs`, the 706 User's Manual §5-9 disc). A file that isn't there is a drive that was never installed and stays silent; an image must be exactly 770,048 bytes (385,024 words — make a blank with `python3 -c "open('ray703-disc0.img','wb').truncate(770048)"`), and writes go through to the file. Gitignored like the floppy.
