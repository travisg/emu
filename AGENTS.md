...
**REX** (`test/703/rex.asm`, ~925 words across five pages) is a preemptive round-robin executive on the invented 60 Hz line clock. Three tasks print a letter and sleep; a fourth is a shell — `HELP`, `STAT`, `UPTIME`, `STOP`/`START` (with a task letter, or bare for all three), `ECHO`, `HALT` — and the level-2 timer interrupt switches between them all. A context is four words, ACR, IXR and the hardware-saved PC and status, so the switch is a handful of word copies and an `INR 2`; the status word carries EXR, the indicators and the addressing mode, which is why a task can be preempted between an `SMB` and its reference, or a compare and its skip, and resume intact. Each block carries two more words, a state and a delay, which is the whole of **sleep**: with no instruction to yield with, a task marks itself sleeping and spins on its own state, the scheduler counts the delay down every tick and marks it runnable at zero, and the scan passes over it meanwhile. When every task is asleep the **idle task** — a branch to self — has the processor, which in a traced run is most of the time.

Four rules hold it together, each stated once in the source header. The scheduler **defers** whenever the saved PC lies inside the service-routine range: a tick there interrupted the driver, not a task, and tasks reach that range only under `MSK`, which is what makes the test exact. Every service routine **leads with `SMB`**, because the entry sequence does not reload EXR (see below). A **sleep spin must live outside** that same range, or the tick could never take the processor away from it. And the line buffer is **born primed with its ready flag set**, so a keystroke landing before the shell first opens it is dropped rather than stored through a pointer that is still byte address zero — which is the level 0 program counter save, and is exactly the bug `basic.asm` has.
