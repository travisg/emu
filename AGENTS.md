...
**REX** (`test/703/rex.asm`, ~300 words across four pages) is a preemptive round-robin executive on the invented 60 Hz line clock: three static tasks print their letters through a mailbox teletype driver, and the level-2 timer interrupt switches between them. A context is four words — ACR, IXR and the hardware-saved PC and status — so the switch is a handful of word copies and an `INR 2`; the status word carries EXR, the indicators and the addressing mode, which is why a task can be preempted between an `SMB` and its reference, or a compare and its skip, and resume intact. Each task's block carries two more words, a state and a delay, which is the whole of **sleep**: with no instruction to yield with, a task marks itself sleeping and spins on its own state, the scheduler counts the delay down on every tick and marks it runnable at zero, and the scan passes over it meanwhile. When every task is asleep the **idle task** — a branch to self, outside the deferred range like any task's code — has the processor. The two rules a reader needs are stated in its header: the scheduler **defers** whenever the saved PC lies inside the service-routine range — a tick there interrupted the driver, not a task, and tasks only enter that range under `MSK`, which is what makes the check exact — and every service routine **leads with `SMB`**, because the entry sequence does not reload EXR (see below). `make -C test ray703-rex` builds it; `./target/debug/emu -s ray703 -r roms/703/rex.bin --fast-io` shows a run of each letter per 9,523-cycle slice, `'.'` shuts it down cleanly; `make -C test ray703-rex-test` is the scripted session — the first 703 harness to use `--fast-io` (the slices stay real machine time; the clock ignores the flag) and a `-l` instruction-limit hang guard.
