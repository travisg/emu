...
- Adding a `panel` token (`ray703-panel`, `ray703-panel-ptb`) opens the front panel from figure 5-1 of the manual in an SDL window: the PROGRAM COUNTER lamps, the SELECTED DISPLAY row behind the six-position DISPLAY SELECTOR (click the knob or a label, or Tab), and the four SENSE toggles (click, or keys 1–4), which then really drive the SS0–SS3 skips. Display only beyond that — RUN/HALT/RESET and data entry are future work. The teletype stays on the terminal; keys into the panel window are deliberately not forwarded to the guest. Panel machines default `--throttle` on at the 703's 4/7 MHz so the lamps move at period speed; `ray703-panel-ptb --throttle 10` watching the index register crawl through a tape load is the demo.
...
Deliberate divergences, all documented at their use sites: `HLT` exits the emulator, because there is no front panel RUN switch to resume from (the panel window doesn't change this yet); the external sense line reads false so SSE always skips, and without a panel the four sense switches read false too — with one, SS0–SS3 read the SENSE toggles for real; the optional multiply/divide hardware decodes as `BadOpcode`, since appendix B lists no opcodes for it; a `DOT` completes on the next instruction rather than at ten characters a second — `--throttle` paces the whole machine but does not slow the device model. The saved machine status word's bit layout is the one thing here not taken from the manual — the manual names its contents but never diagrams it, so the layout follows rustheon's (and it turned out to be figure 5-1's MS lamp layout, see `publish_panel`).
