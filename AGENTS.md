...
- `SUB`/`SBC` on **both** Motorola cores negated the operand and added, carrying the C++'s own "XXX make sure carry is okay" comment. The carry was fine; V was not, because a 32-bit negation sets every bit above 7 of the operand and both cores' V expressions read one of those bits. On the 6800 that inverted V for an operand of exactly `0x80`; on the 6809, whose `set_v1` also takes the operand's own bit 7, it inverted V for 65,024 of the 65,536 byte operand pairs — every subtraction of a non-zero byte, at either carry, and the same at 16 bits. `CMP` was always right on both, because it never negated, and `SUB` now works the same way. Pinned by exhaustive tests over every operand pair and both carry inputs (`sub_and_sbc_flags_match_the_datasheet_for_every_operand` in each core, plus `sba_flags_...` and `subd_flags_...`)
